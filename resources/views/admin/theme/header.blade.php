            ...
            <div class="position-relative mx-1">
                <div class="dropdown d-lg-block d-none">
                    <a class="btn btn-sm border-primary dropdown-toggle" href="javascript:void(0)" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        <img src="{{ helper::image_path(session()->get('flag')) }}" alt="" class="mx-1 rounded-5 language-width">
                        <span class="{{ Session::get('theme') == 'dark' ? 'text-white' : '' }}">{{ session()->get('language') }} </span>
                    </a>
                    <ul class="dropdown-menu drop-menu {{ session()->get('direction') == 2 ? 'drop-menu-rtl' : 'drop-menu'}}">
                        @foreach(Helper::language() as $lang)
                        <li>
                            <a class="dropdown-item d-flex text-start d-flex" href="{{ URL::to('/language-'.$lang->code)}}">
                                <img src="{{ helper::image_path($lang->image) }}" alt="" class="img-fluid mx-1 rounded-5 language-width">
                                {{ $lang->name }}
                            </a>
                        </li>
                        @endforeach
                ...
                <div class="dropdown d-block d-lg-none">
                    ...
                    <ul class="dropdown-menu {{session()->get('direction') == '2' ? 'min-dropdown-rtl' : 'min-dropdown'}}" aria-labelledby="dropdownMenuButton1">
                        @foreach(Helper::language() as $lang)
                        <li><a class="dropdown-item text-dark d-flex" href="{{ URL::to('/language-'.$lang->code)}}"><img src="{{ Helper::image_path($lang->image)}}" class="img-fluid lag-img mx-1 rounded-5 language-width" alt="">{{ $lang->name}}</a></li>
                        @endforeach
                    </ul>
                </div>
                <!-- language-btn -->
